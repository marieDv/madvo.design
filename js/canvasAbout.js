...
let posY = Math.floor(Math.random() * window.innerWidth) /50;

let posXTwo = Math.floor(Math.random() * window.innerWidth) /50;
let posXThree = Math.floor(Math.random() * window.innerWidth) /50;
let posXBefore = posX;
/**
 *render and camera
 */
let renderer = new THREE.WebGLRenderer({ canvas: document.getElementById("three-about") });
...
let addtocanvas = 0;
let porportions = document.getElementById("three-about");
...
function scene1() {
  ...
  /////// MORPH TARGET
  morphTarget = new THREE.IcosahedronBufferGeometry(WIDTH / 2, 2);
  ...
  cube = new THREE.Mesh(sphereGeometry, shaderMaterial);
  let cubeCopy = new THREE.Mesh(sphereGeometry, shaderMaterial);
  let cubeCopyCopy = new THREE.Mesh(sphereGeometry, shaderMaterial);;
  ...
  cubeCopy.matrixAutoUpdate = true;

animateCubes(cubeCopy, posX);
animateCubes(cube, posXTwo);
animateCubes(cubeCopyCopy, posXThree);



function animateCubes(cubeCopy, seed){
 // posX = Math.sin(Math.floor(Math.random() * (window.innerWidth - -window.innerWidth) + -window.innerWidth * window.innerWidth) *seed );
  ...
  tl.to(cubeCopy.position, 180, {
    ...
    //  cubeCopy.position.y = Math.floor( Math.random() * window.innerHeight );
        tl.invalidate();
        cubeCopy.position.x = seed;
        cubeCopy.position.y = posY;
    }
  });
}
